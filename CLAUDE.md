...
**FORK STATUS:** this repo is **Skewbiks.com**, a Skewb sister site forked from `pyraminx-oo` (the `upstream` remote). Port progress and the remaining milestone plan live in **`docs/port-plan.md`**; machine-verified Skewb domain facts (move tables, state space, symmetries, test vectors, sources) live in **`docs/skewb-ground-truth.md`** — read both before working on engine/renderer/sheet/trainer/solver code. As of 2026-07-03: **M0–M3 are done** (Skewb engine behind the same `window.OOEngine` surface, Skewb renderer, working OO census — Home and oo.html are fully functional); **algs.html, trainer.html and solver.html are knowingly broken** until their milestones (M5/M6/M7), and `npm test`/`npm run build`'s check step is red until M5 (the shipped `js/sheet.js` is still Pyraminx data — use `npm run stamp` and `npm run test:engine`/`test:space` meanwhile). The port strategy: the Skewb engine implements the **same `window.OOEngine` member surface** so engine-abstracted consumers (`oo.js`, `tables.js`, `solver.js` UI, `tools/lib/bfs-dist.mjs`) port nearly unchanged. Internal names (`OOEngine`, `OO_CONFIG`, `oo.html`, `oo-*`) are deliberately NOT renamed, to keep upstream cherry-picks clean (`git fetch upstream` + cherry-pick for shared-layer fixes; the repo pins `core.autocrlf=false` — don't let a CRLF checkout poison the hashes). Everything below this paragraph describes the codebase as inherited; where a milestone has landed, the engine/renderer specifics are Skewb (see the two docs).
